...
class SiteController extends Controller
{
    ...
    public function storePipeline(Request $request, CreateSiteService $createSite, ProvisionWordPressService $provision)
    {
        $this->authorizePipeline();
        ...
        $data = $request->validate([
            ...
            'profile_id' => ['nullable', 'in:p01,p02,p03,p04,p05,p06,p07,p08'],
            'group_id' => ['nullable', 'exists:site_groups,id'],
            'git_ref' => ['nullable', 'string', 'max:100'],
        ...
        if ($request->boolean('provision_now')) {
            ...
            return redirect()
                ->route('deployments.runs.show', $run)
                ->with('ok', 'Site created. Provision queued as run #'.$run->id);
        }

        return redirect()->route('sites.show', $site)->with('ok', 'Pipeline site created (pins + targets). Provision separately when docroot is ready.');
    }
    ...
    private function authorizePipeline(): void
    {
        $user = auth()->user();
